...
QUERY_FLIGHT_BY_ID = """
    ...
    JOIN
        airlines
        ON flights.airlines = airlines.id
    WHERE
        flights.ID = :id
"""
...
QUERY_DELAYED_FLIGHTS_BY_AIRLINE = """
    ...
    WHERE
        airlines.airline LIKE :airline AND flights.DEPARTURE_DELAY > 0
"""
...
QUERY_DELAYED_FLIGHTS_BY_AIRPORT = """
    ...
    JOIN
        ...
        ON flights.airline = airlines.id
    WHERE
        flights.ORIGIN_AIRPOST = :airport AND flights.DEPARTURE_DELAY > 0
"""
...
QUERY_FLIGHTS_BY_DATE = """
    SELECT
        flights.ORIGIN_AIRPORT, flights.DESTINATION_AIRPORT,
        flights.DEPARTURE_DELAY as DELAY, airlines.airline, flights.FLIGHT_DATE
    FROM
        flights
    JOIN
        airlines
